...
- Schema `VideoObject.uploadDate` → NE JAMAIS le générer avec `Date.now()` (date instable, recalculée à chaque build) ni en date seule `YYYY-MM-DD`. Sinon Search Console signale "fuseau horaire manquant" + "valeur de date incorrecte" (Google compare le schema à la vraie vidéo YouTube). Toujours une date FIXE et RÉELLE en ISO 8601 AVEC fuseau (ex `2026-05-28T05:30:05-07:00`), récupérée de la page YouTube `watch?v=ID` (elle expose `"uploadDate":"..."`). Corrigé le 15/06/2026 sur `app/(marketing)/tuto-video/page.tsx`.
- Pages programmatiques « villes » (SEO local) → 1 seul paragraphe unique + corps identique sur 88 pages = ~97 % de contenu dupliqué. Sur un domaine jeune / à faible autorité, Google n'en indexe qu'une poignée (statut « Détectée/Explorée, actuellement non indexée » dans GSC ; constaté 25/06/2026 à 23/77 pages indexées). Fix : champ optionnel `localSections` (3 blocs de VRAIE donnée locale sourcée par commune — quartiers, médiathèques, patrimoine, accès) + `sources`, rendus dans `app/(marketing)/[ville]/page.tsx`, appliqués aux 15 villes prioritaires. Recherche via **Perplexity `sonar-pro`** (le modèle `sonar` simple est trop pauvre) ; clé API jamais écrite dans le repo. NB : le MCP data.gouv (Pappers) était à court de crédits — contourné par Perplexity. Le vrai levier d'indexation reste l'autorité (liens, GBP, avis) : viser « toutes les pages indexées » est un mauvais objectif, mieux vaut 20-30 pages qui ramènent des clients.
